function updateCard(form, data, card){
	form.find('button[type="submit"][name="update"]').unbind().click(function(e){
		...
		.then(r=>{
			...
			swal.showLoading();
			customFetch(json, "PUT", `/Trello/CardServlet?board_id=${json.board_id}&card_id=${json.card_id}`)
			.then(r=>{
				if(r.status === 200){
					swal({
	...
	form.find('button[type="submit"][name="delete"]').unbind().click(function(){
		swal({
			...
			if (result.value) {
				...
				json.board_id = parseInt(new URLSearchParams(window.location.search).get("board_id"));
				swal.showLoading();
				customFetch(json, "DELETE", `/Trello/CardServlet?board_id=${json.board_id}&card_id=${json.card_id}`)
				.then(r=>{
					if(r.status === 200){
						swal({
